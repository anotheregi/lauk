...
-- ================== DATA IKAN & FILTER ==================
local FishData = {}          -- key: nama ikan, value: tier
                ...
                if success and type(module) == "table" then
                    ...
                    if module.Data and type(module.Data) == "table" and module.Data.Type == "Fish" then
                        ...
                        if fishName and tier then
                            fishData[string.lower(fishName):gsub("^%s*(.-)%s*$", "%1")] = tonumber(tier)
                        end
...
local function getFishIconURL(fishName)
    ...
    -- 🔎 Scan semua ModuleScript
    for _, moduleScript in ipairs(itemsFolder:GetChildren()) do
        if moduleScript:IsA("ModuleScript") then
            local success, fishData = pcall(require, moduleScript)

            if success and type(fishData) == "table" and fishData.Data then
                local data = fishData.Data

                -- Cocokkan dengan Data.Name (bukan nama module)
                if data.Name == fishName then

                    -- 🔒 Hanya Secret
                    local tier = tonumber(data.Tier) or 0
                    if tier < TIER.Secret then
                        IconCache[fishName] = DEFAULT_THUMBNAIL
                        return DEFAULT_THUMBNAIL
                    end

                    local assetString = data.Icon
                    local assetId = type(assetString) == "string" and assetString:match("%d+")
                    ...
                    local finalUrl = cdnUrl or DEFAULT_THUMBNAIL

                    IconCache[fishName] = finalUrl
                    return finalUrl
                end
            end
        end
            ...
            remote.OnClientEvent:Connect(function(...)
                ...
                for _, arg in ipairs(args) do
                    if type(arg) == "string" and string.find(arg, "obtained a") then
                        ...
                        -- Bersihkan nama ikan
                        local cleanName = string.lower(parsed.fishName):gsub("^%s*(.-)%s*$", "%1")

                        -- Lookup langsung (O(1))
                        local tier = FishData[cleanName]
                        local requiredTier = TIER[SelectedFilter]

                        print("[Debug] Fish:", parsed.fishName, "Tier:", tier, "Required:", requiredTier)
